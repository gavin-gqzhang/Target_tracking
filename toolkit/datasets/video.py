...
class Video(object):
    def __init__(self, name, root, video_dir, init_rect, img_names,
        ...
        self.attr = attr
        self.pred_trajs = {}
        # self.img_names = [os.path.join(os.path.abspath(root), os.path.abspath('testing_dataset/VOT2018/'+str(x).split('/')[-1])) for x in img_names]
        ...
        if load_img:
            ...
            for img_name in self.img_names:
                prints=0
                img=Image.open("{}/{}".format("testing_dataset/OTB",img_name))
                img=np.array(img)
                try:
                    channel=img.shape[2]
                except IndexError as e:
                    # 检测到灰度图，将其转化为三通道图像
                    img=cv2.imread("{}/{}".format("testing_dataset/OTB",img_name))
                    img=np.array(img)

                imgs.append(img)
                assert img is not None,"{} 读取图片失败 ".format(img_name)
    ...
    def __iter__(self):
        for i in range(len(self.img_names)):
            if self.imgs is not None:
                yield self.imgs, self.gt_traj[i]
            else:
