...
class OPEBenchmark:
    ...
    def eval_success(self, eval_trackers=None):
        ...
        for tracker_name in eval_trackers:
            ...
            video=self.dataset.videos[tracker_name]
            gt_traj=video.gt_traj
            try:
                tracker_traj=video.pred_trajs[tracker_name]
            except:
                print('load tracker from {}.txt'.format(tracker_name))
            ...
            success_ret[tracker_name]=success_overlap(gt_traj, tracker_traj, n_frame)
        return success_ret
    ...
    def eval_precision(self, eval_trackers=None):
        ...
        for tracker_name in eval_trackers:
            ...
            precision_ret[tracker_name] = success_error(gt_center, tracker_center,
                                                       thresholds, n_frame)
    ...
    def show_result(self, success_ret, precision_ret=None,
        ...
        for tracker_name in success_ret.keys():
            auc = np.mean(list(success_ret[tracker_name]))
        ...
        for tracker_name in tracker_names:
            ...
            if precision_ret is not None:
                precision = np.mean(list(precision_ret[tracker_name]), axis=0)
            else:
                precision = 0
